...
def form_field(label, input_id, input_type="number", placeholder="", value=""):
    return dbc.Col([
        ...
        dbc.Input(
            ...
            value=value, className="form-input-custom",
        ),
...
def empty_state(message="No data yet. Head to Admin to add teams and enter match scores!"):
    return html.Div([
...
def season_progress(current_match, total_matches):
    ...
    return html.Div([
        html.Div([
            html.Span(f"Match {current_match} of {total_matches}", className="progress-label"),
            html.Span(f"{pct:.0f}%", className="progress-pct"),
        ], className="progress-header"),
        dbc.Progress(
            value=pct, className="season-progress",
...
def leaderboard_row(rank, team, total_points, rank_change, gap, color):
    ...
    return html.Div([
        html.Div([
            ...
            html.Div(className="lb-color-dot", style={"backgroundColor": color}),
